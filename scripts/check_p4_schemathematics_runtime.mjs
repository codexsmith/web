...
async function waitForServer() {
  ...
  throw new Error(`Timed out waiting for Schemathematics runtime server\n${output}`);
}
...
try {
  ...
  if (detail.status !== 200) {
    throw new Error(`Schemathematics detail route returned HTTP ${detail.status}`);
  }
  ...
  const expected = [
    ...
    "Operative profile",
    "Inspection grammar",
    "Established mathematics witness",
    "Worked comparison: monoid",
    "Universal invertibility",
    "Research hypotheses",
    "Standing · hypothesis",
    "Validation / proof burden",
    "Safe public standing",
    "Not yet established",
    "Return to object",
  ];
  ...
  for (const marker of expected) {
    if (!html.includes(marker)) {
      throw new Error(`Schemathematics detail route did not contain expected marker: ${marker}`);
    }
  }

  for (const forbidden of ["Structured retained record", "Governed public landing"]) {
    if (html.includes(forbidden)) {
      throw new Error(`Schemathematics detail route contained forbidden fallback marker: ${forbidden}`);
    }
  }

  console.log("P4 Schemathematics production formal program: pass");
} finally {
  await stopServer();
}
