...
The first public Experiment-object cohort is the seven durable `EXP-*` seed records projected from the Lab-wide Experiment Register into `content/experiments.ts`. Unlike Evidence surface keys, `EXP-*` values are source-governed durable experiment identities and should be rendered as such. The website pins the Lab source revision used for the static projection, preserves source status and result posture without translating them into a universal maturity scale, and keeps the Experiment Register authority ceiling visible. Experiment-to-Research edges are admitted only from explicit `RL-*` lane links recorded in the central register; shared terminology, program ancestry, or conceptual fit is insufficient.

The first public Machinery-object cohort is the ten `BFL-MACH-*` components projected from the Lab Machinery Registry into `content/machinery.ts`. Component identity, maturity, integration level, manifest status, side-effect class, entrypoints, durable projections, authority ceiling, and next integration step remain separate fields. Registry membership supports discovery/routing and must not be translated into runtime permission, scientific authority, publication promotion, or institutional promotion. The current machinery registry does not declare object-level relationships to the other public Atlas families, so the first machinery cohort creates no inferred Atlas edges.

The first public Claim-object cohort is the eight durable `IM-C*` rows from the owner-local Information Mechanics Claim Ledger, an active registrar-admitted scientific-state/evidence registry. Claim status, validation requirement, novelty posture, evidence annotations, and source locators remain distinct. The only initial Atlas relationship is Claim -> OWNER PROGRAM -> Information Mechanics because the ledger itself declares `program: information_mechanics`; evidence prose such as “finite countermodels” is searchable support text, not promoted into an Evidence object without durable identity. No Lab-global claim namespace is inferred from the existence of other package-local claim ledgers.
...
The Apparatus route uses `sections/ApparatusContextSection.tsx`: source-bound registered `BFL-MACH-*` components are rendered directly as the canonical machinery-object surface; the Instrument Bench remains directly readable as the public apparatus vocabulary; every supporting Apparatus section is compressed into a nine-object focus-stage Reflow field. Registered machinery and conceptual instruments are deliberately not collapsed into one object family.
...
The Representation Atlas at `/v3/representation-atlas` is a separate public comparative instrument from the feature-frozen Lab Atlas. The Lab Atlas maps source-governed institutional objects and declared relationships; the Representation Atlas holds a small mechanics grammar fixed while switching among five source-backed witness domains. Its current mechanics slots are Frame, Representation, Admissible Change, Invariant, Returned Defect, and Repair / Test. Its current witness domains are Social Meaning, Public Knowledge, Formal Strategy, Embodied Agency, and Physical Dynamics. The cross-domain cells are working analytical questions, not claims of formal equivalence, empirical validation, or transferred domain authority. The page pins the Lab source revision and links the public Witness Domains artifact and the unpromoted candidate Representation Mechanics spine.


The v3 publication catalog is now source-governed across three distinct control authorities rather than website-local status shorthand. `REG-PUBLICATION-SEQUENCE` supplies governance/dependency state, `REG-PUBLICATION-GRAPH` supplies routing/readiness projections, and `REG-PUBLICATION-SOURCES` supplies source identity/provenance routing. The public selection may orient and prioritize records for first contact, but it must preserve the source identity and authority ceiling of the controlling registry. Sequence status, graph readiness, source registration, and website selection must never be rendered as synonymous with published, peer reviewed, true, novel, or release-authorized. Publication cards deep-link by source record ID and expose pinned source revisions, control fields, and authority ceilings.
