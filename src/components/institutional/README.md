...
Institutional navigation distinguishes global routes from contextual child routes. Child pages are not repeated in the top-level header navigation; parent routes expose them through the shared lower-right hero child-page rail. The footer deliberately exposes stable child destinations directly alongside the global routes. Apparatus is reachable as a child of both Research and Open Lab while retaining its canonical `/v3/apparatus` route. Funding is reachable as a child of About, Research, and Open Lab while retaining its canonical `/v3/funding` route. Founder is a child of About at `/v3/founder`. Funding, Apparatus, and Founder are also direct footer destinations.
