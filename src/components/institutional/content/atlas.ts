...
const publicationNodes: AtlasNode[] = selectedPublications.map((publication) => ({
  ...
  href: `/publications#publication-${publication.id}`,
...
const evidenceNodes: AtlasNode[] = priorExecution.map((record) => ({
  ...
  kind: "evidence",
  title: record.title,
  href: `/evidence#evidence-${record.surfaceKey}`,
...
const experimentNodes: AtlasNode[] = experimentRecords.map((experiment) => ({
  ...
  href: `/experiments#experiment-${experiment.id.toLowerCase()}`,
...
const apparatusNodes: AtlasNode[] = machineryRecords.map((machine) => ({
  ...
  kind: "apparatus",
  title: machine.name,
  href: `/apparatus#machinery-${machine.machineId.toLowerCase()}`,
...
const claimNodes: AtlasNode[] = claimRecords.map((claim) => ({
  ...
  kind: "claim",
  title: claim.claim,
  href: `/claims#claim-${claim.id.toLowerCase()}`,
  identifier: claim.id,
