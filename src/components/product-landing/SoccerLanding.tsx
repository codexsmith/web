...
const passIcons = [Radar, MoveRight, GitBranch, ArrowRight, CornerDownLeft] as const;
...
export function SoccerLanding() {
  ...
  return (
    <main className="min-h-screen bg-background text-foreground">
      ...
      <section className="relative isolate overflow-hidden border-b border-border bg-[#0d2117] px-5 py-16 text-brand-ivory sm:px-8 sm:py-24">
        <div className="pointer-events-none absolute inset-0 -z-20 [background-image:linear-gradient(rgba(248,243,232,.05)_1px,transparent_1px),linear-gradient(90deg,rgba(248,243,232,.05)_1px,transparent_1px)] [background-size:56px_56px]" />
        <div className="pointer-events-none absolute -right-20 top-20 -z-10 h-72 w-[32rem] rounded-[50%] border border-white/10" />
        <div className="mx-auto grid max-w-7xl gap-12 lg:grid-cols-[minmax(0,1.08fr)_minmax(20rem,0.62fr)] lg:items-end">
          ...
          <aside className="border border-white/15 bg-white/[0.045] p-6 backdrop-blur-sm sm:p-7">
            ...
            <div className="mt-6 grid grid-cols-2 gap-px overflow-hidden border border-white/15 bg-white/15">
              {fieldLens.dimensions.map((dimension, index) => (
                <div className="bg-[#0d2117] p-4" key={dimension.name}>
                  <span className="font-mono text-[9px] text-white/35">
                    {String(index + 1).padStart(2, "0")}
                  </span>
                  <p className="mt-3 font-serif text-lg font-semibold text-white/90">{dimension.name}</p>
                </div>
              ))}
            </div>
      ...
      <PublicLandingRail currentId="boundary-first-soccer" />
      ...
      <section className="border-b border-border bg-card/45 px-5 py-14 sm:px-8 sm:py-20" id="method">
        <div className="mx-auto max-w-7xl">
          <div className="max-w-4xl">
            <p className="font-mono text-[10px] font-semibold uppercase tracking-[0.18em] text-foreground-muted">Team-state method</p>
            ...
            {method.passes.map((pass, index) => {
              ...
              return (
                <article className="border border-border bg-background p-5" key={pass.name}>
                  ...
                  <h3 className="mt-7 font-serif text-2xl font-semibold leading-8">{pass.name.replace(/^\d+\.\s*/, "")}</h3>
                  <p className="mt-4 text-sm leading-7 text-foreground-muted">{pass.question}</p>
                  <p className="mt-6 border-t border-border pt-4 font-mono text-[9px] font-semibold uppercase tracking-[0.12em] text-foreground-muted">Output · {pass.output}</p>
                </article>
              );
            })}
          </div>
      ...
      <section className="scroll-mt-24 border-b border-border px-5 py-14 sm:px-8 sm:py-20" id="worked-example">
        <div className="mx-auto max-w-7xl">
          <p className="font-mono text-[10px] font-semibold uppercase tracking-[0.18em] text-foreground-muted">Constructed possession trace</p>
          ...
          <div className="mt-9 grid gap-3 lg:grid-cols-[0.65fr_1.35fr]">
            <article className="border border-border bg-card p-6 sm:p-8">
              <p className="font-mono text-[9px] font-semibold uppercase tracking-[0.14em] text-foreground-muted">Naive read</p>
            ...
            <article className="border border-border bg-primary p-6 text-primary-foreground sm:p-8">
              <p className="font-mono text-[9px] font-semibold uppercase tracking-[0.14em] text-primary-foreground-muted">Boundary trace</p>
              <ol className="mt-5 grid gap-3">
                {workedExample.boundaryTrace.map((item, index) => (
                  <li className="grid grid-cols-[2rem_1fr] gap-3 text-sm leading-7 text-primary-foreground-secondary" key={item}>
          ...
          <div className="mt-5 grid gap-4 md:grid-cols-2">
            <p className="border-l-2 border-accent pl-5 text-sm leading-7 text-foreground-muted"><strong className="text-foreground">Promotion test:</strong> {workedExample.promotionTest}</p>
            <p className="border-l-2 border-border pl-5 text-sm leading-7 text-foreground-muted">{workedExample.lesson}</p>
          </div>
          <p className="mt-4 text-xs leading-6 text-foreground-muted">{workedExample.scopeNote}</p>
        </div>
      </section>
      ...
      <section className="border-b border-border bg-card/55 px-5 py-14 sm:px-8 sm:py-20">
        <div className="mx-auto grid max-w-7xl gap-8 lg:grid-cols-[0.85fr_1.15fr]">
          <div>
            <p className="font-mono text-[10px] font-semibold uppercase tracking-[0.18em] text-foreground-muted">Analyst vocabulary</p>
            <h2 className="mt-4 font-serif text-4xl font-semibold">{taggingGrammar.title}</h2>
            <p className="mt-5 text-sm leading-7 text-foreground-muted">{taggingGrammar.rule}</p>
          </div>
          <div className="grid gap-2 sm:grid-cols-2 lg:grid-cols-3">
            {taggingGrammar.tags.map((tag, index) => (
              <div className="border border-border bg-background p-4" key={tag}>
                <span className="font-mono text-[9px] text-foreground-muted">{String(index + 1).padStart(2, "0")}</span>
                <p className="mt-3 font-serif text-lg font-semibold">{tag}</p>
              </div>
            ))}
          </div>
        </div>
      ...
      <section className="border-b border-border px-5 py-14 sm:px-8 sm:py-20">
        <div className="mx-auto grid max-w-7xl gap-8 lg:grid-cols-2">
          ...
          <article className="border border-border bg-card p-6 sm:p-8">
            <p className="font-mono text-[10px] font-semibold uppercase tracking-[0.18em] text-foreground-muted">Built so far</p>
            <ul className="mt-6 grid gap-3">
              {proof.built.map((item) => <li className="border border-border bg-background p-4 text-sm font-medium" key={item}>{item}</li>)}
            </ul>
      ...
      <section className="border-b border-border bg-[#10261b] px-5 py-14 text-brand-ivory sm:px-8 sm:py-20">
