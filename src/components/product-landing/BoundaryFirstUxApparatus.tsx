...
export function BoundaryApparatus({ state, actions }: { state: ApparatusState; actions: ApparatusActions }) {
  ...
  return (
    <div className="relative overflow-hidden border border-white/15 bg-[#071321] p-4 text-brand-ivory shadow-[inset_0_0_80px_rgba(0,0,0,.28)] sm:p-5">
      ...
      <div className="relative grid gap-5 xl:grid-cols-[13.5rem_minmax(0,1fr)_14rem] xl:items-start">
        ...
        <div className={`relative border p-4 transition-colors sm:p-5 ${state.promoted ? "border-brand-blue/60 bg-brand-blue/[.035]" : "border-white/12 bg-white/[.02]"}`}>
          ...
          <div className="mx-auto mt-6 grid max-w-[38rem] grid-cols-[8.5rem_minmax(12rem,1fr)_8.5rem] grid-rows-[auto_8rem_auto] items-center gap-x-3 gap-y-3">
            ...
            <div className="flex h-full items-center justify-center">
              <div className="text-center">
                <p className="font-mono text-[8px] font-semibold uppercase tracking-[.1em] text-white/35">Stress station</p>
                <p className="mt-1 text-[11px] leading-4 text-white/50">Load the handoff;<br />observe the world.</p>
              </div>
            </div>
        ...
        <aside className="space-y-3">
          <p className="font-mono text-[8px] font-semibold uppercase tracking-[.12em] text-white/35">Semantic material key</p>
          ...
          <LegendRow icon={<Gauge />} label="Gauge / recorder" meaning="Observable state and remembered path" />
...
function Vessel({ title, subtitle, active, muted, compact }: { title: string; subtitle: string; active?: boolean; muted?: boolean; compact?: boolean }) {
  return (
    <div className={`relative mx-auto w-full max-w-[15rem] overflow-hidden rounded-[2.8rem_2.8rem_1.2rem_1.2rem] border-2 text-center transition-all ${compact ? "min-h-32" : "min-h-44"} ${active ? "border-brand-gold bg-white/[.08] shadow-[inset_0_-35px_65px_rgba(37,99,235,.13)]" : "border-white/20 bg-white/[.03]"} ${muted ? "opacity-35" : ""}`}>
      ...
      <div className="relative z-10 px-5 pb-5 pt-8">
        ...
        <span className="mx-auto mt-2 block max-w-36 text-xs leading-5 text-white/55">{subtitle}</span>
      </div>
      <div className={`absolute bottom-0 left-3 right-3 rounded-t-[50%] transition-all ${active ? compact ? "h-9 bg-brand-blue/25" : "h-12 bg-brand-blue/25" : "h-5 bg-white/[.04]"}`} aria-hidden="true" />
    </div>
  );
}

function PortRail({ side, label, detail, active }: { side: "left" | "right"; label: string; detail: string; active: boolean }) {
  const content = <div className={side === "left" ? "text-right" : "text-left"}><span className="block font-mono text-[8px] font-semibold uppercase tracking-[.1em] text-white/55">{label}</span><span className="mt-1 block text-[10px] leading-4 text-white/35">{detail}</span></div>;
  ...
  const pipe = <span className={`h-2 min-w-6 flex-1 rounded-full border border-black/25 ${active ? "bg-brand-gold shadow-[0_0_14px_rgba(200,162,74,.22)]" : "bg-white/12"}`} />;
  return <div className="flex w-full items-center gap-2">{side === "left" ? <>{content}{port}{pipe}</> : <>{pipe}{port}{content}</>}</div>;
}
...
function VerticalTransfer({ state, onReveal, onRepair }: { state: ApparatusState; onReveal: () => void; onRepair: () => void }) {
  ...
  return (
    <div className="relative mx-auto flex h-full min-h-28 w-36 items-center justify-center">
      ...
      {defect ? (
        <button type="button" onClick={onRepair} className="absolute left-[calc(50%+1rem)] top-[62%] flex items-center" aria-label="Repair boundary leak">
          <span className="h-1 w-9 bg-brand-red shadow-[0_0_14px_rgba(159,45,32,.6)]" />
      ...
      {state.repaired ? (
        <button type="button" onClick={onRepair} className="absolute left-[calc(50%+1rem)] top-[62%] flex items-center text-brand-green" aria-label="Remove repair clamp">
          <span className="h-1 w-9 bg-brand-green" />
...
function Prism({ active, disabled, onClick }: { active: boolean; disabled: boolean; onClick: () => void }) {
  return (
    <button type="button" disabled={disabled} onClick={onClick} aria-pressed={active} className="mx-auto text-center disabled:cursor-not-allowed disabled:opacity-25">
      <span className={`mx-auto block h-0 w-0 border-b-[56px] border-l-[32px] border-r-[32px] border-l-transparent border-r-transparent transition-all ${active ? "border-b-brand-blue drop-shadow-[0_0_16px_rgba(37,99,235,.4)]" : "border-b-white/20"}`} />
...
function LoadRig({ active, disabled, onClick }: { active: boolean; disabled: boolean; onClick: () => void }) {
  return (
    <button type="button" disabled={disabled} onClick={onClick} aria-pressed={active} className={`w-full border p-4 text-left transition-colors disabled:cursor-not-allowed disabled:opacity-30 ${active ? "border-brand-red/70 bg-brand-red/10" : "border-white/12 bg-white/[.035] hover:border-white/30"}`}>
      <div className="flex items-center gap-3"><ShieldAlert className={`h-5 w-5 ${active ? "text-brand-red" : "text-brand-gold"}`} aria-hidden="true" /><div><span className="font-mono text-[8px] font-semibold uppercase tracking-[.12em] text-white/40">Load rig</span><strong className="mt-1 block text-sm">{active ? "Stress applied" : "Apply stress"}</strong></div></div>
...
function ProvenanceTape({ state }: { state: ApparatusState }) {
  ...
  return (
    <div className="mt-6 border-t border-white/10 pt-4">
      <div className="flex items-center justify-between gap-3"><p className="font-mono text-[8px] font-semibold uppercase tracking-[.12em] text-white/35">Recorded state / provenance tape</p><span className="font-mono text-[8px] text-white/30">{events.length} marks</span></div>
      <div className="mt-3 flex min-h-10 items-center overflow-x-auto rounded-full border border-white/15 bg-black/20 px-3">
        {events.map((event, index) => <span key={event} className="inline-flex min-w-max items-center font-mono text-[8px] font-semibold uppercase tracking-[.1em] text-white/55"><span className="mr-2 h-2 w-2 rounded-full bg-brand-gold" />{event}{index < events.length - 1 ? <span className="mx-3 h-px w-8 bg-white/20" /> : null}</span>)}
      </div>
    </div>
  );
}
