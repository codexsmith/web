...
import { LabMachineAboutProjection, isAboutProjectionMode, type AboutProjectionMode } from "./LabMachineAboutProjection";
import { LabMachineApplicationsProjection, isApplicationsProjectionMode, type ApplicationsProjectionMode } from "./LabMachineApplicationsProjection";
import { LabMachineGovernanceProjection, isGovernanceProjectionMode, type GovernanceProjectionMode } from "./LabMachineGovernanceProjection";
import { LabMachineMethodProjection, isMethodProjectionMode, type MethodProjectionMode } from "./LabMachineMethodProjection";
import { LabMachinePeopleProjection, isPeopleProjectionMode, type PeopleProjectionMode } from "./LabMachinePeopleProjection";
import { LabMachinePipelineProjection, isPipelineProjectionMode, type PipelineProjectionMode } from "./LabMachinePipelineProjection";
import { LabMachineProductsProjection, isProductProjectionMode, type ProductProjectionMode } from "./LabMachineProductsProjection";
import { LabMachinePublicationsProjection, isPublicationsProjectionMode, type PublicationsProjectionMode } from "./LabMachinePublicationsProjection";
import { LabMachinePublicValueProjection, isPublicValueProjectionMode, type PublicValueProjectionMode } from "./LabMachinePublicValueProjection";
import { LabMachineResearchProjection, isResearchProjectionMode, type ResearchProjectionMode } from "./LabMachineResearchProjection";
import { LabMachineServiceProjection, isServiceProjectionMode, type ServiceProjectionMode } from "./LabMachineServiceProjection";
import { LabMachineTimelineProjection, isTimelineProjectionMode, type TimelineProjectionMode } from "./LabMachineTimelineProjection";
...
type ActiveProjection =
  ...
  | { subsystem: "publications"; mode: PublicationsProjectionMode }
...
function resolveProjection(subsystem: string, mode: string): ActiveProjection {
  switch (subsystem) {
    case "timeline": return isTimelineProjectionMode(mode) ? { subsystem, mode } : null;
    case "products": return isProductProjectionMode(mode) ? { subsystem, mode } : null;
    case "publications": return isPublicationsProjectionMode(mode) ? { subsystem, mode } : null;
    case "research": return isResearchProjectionMode(mode) ? { subsystem, mode } : null;
    case "method": return isMethodProjectionMode(mode) ? { subsystem, mode } : null;
    case "pipeline": return isPipelineProjectionMode(mode) ? { subsystem, mode } : null;
    case "governance": return isGovernanceProjectionMode(mode) ? { subsystem, mode } : null;
    case "about": return isAboutProjectionMode(mode) ? { subsystem, mode } : null;
    case "people": return isPeopleProjectionMode(mode) ? { subsystem, mode } : null;
    case "applications": return isApplicationsProjectionMode(mode) ? { subsystem, mode } : null;
    case "service": return isServiceProjectionMode(mode) ? { subsystem, mode } : null;
    case "public-value": return isPublicValueProjectionMode(mode) ? { subsystem, mode } : null;
    default: return null;
  }
}
...
export function LabMachineDetailPanel({ node, onClose }: { node: LabMachineNode; onClose: () => void }) {
  ...
  if (activeProjection?.subsystem === "publications") return <LabMachinePublicationsProjection initialMode={activeProjection.mode} onBack={() => setActiveProjection(null)} onClose={onClose} />;
  ...
  return (
    <section className="bf-machine-detail" data-tone={node.tone} aria-label={`${node.label} institutional detail`}>
      ...
      <div className="bf-machine-detail__grid">
        <section className="bf-machine-detail__module"><header><span>01</span><h3>BOUNDARY</h3></header><div className="bf-machine-detail__split"><div><small>CONTAINS</small><ul>{content.boundary.contains.map((item) => <li key={item}>{item}</li>)}</ul></div><div><small>EXCLUDES</small><ul>{content.boundary.excludes.map((item) => <li key={item}>{item}</li>)}</ul></div></div></section>
        <section className="bf-machine-detail__module bf-machine-detail__module--process"><header><span>02</span><h3>PROCESS</h3></header><div className="bf-machine-detail__process"><ProcessColumn label="ENTERS AS" items={content.process.entersAs} /><span className="bf-machine-detail__arrow" aria-hidden="true">→</span><ProcessColumn label="TRANSFORMS THROUGH" items={content.process.transformsThrough} /><span className="bf-machine-detail__arrow" aria-hidden="true">→</span><ProcessColumn label="EXITS AS" items={content.process.exitsAs} /></div></section>
      ...
      <section className="bf-machine-detail__views" aria-label={`${node.label} available views`}>
        ...
        <div>{content.views.map((view) => {
          ...
          return <article key={view.id} data-view-id={view.id}><span>PROJECTION</span><strong>{view.label}</strong><p>{view.purpose}</p><button type="button" disabled={!available} title={available ? `Open ${view.label}` : "Projection component pending"} onClick={() => { if (projection) setActiveProjection(projection); }}>{available ? "OPEN" : "PLANNED"}</button></article>;
        })}</div>
      </section>
...
function TraversalTrail() {
  ...
  return <section className="bf-machine-trail" aria-label="Current Lab Machine traversal"><header><div><small>BOUND PATH</small><strong>{focus?.label ?? navigation.focusLabel} → {getLabMachineNode(navigation.currentNodeId)?.label ?? navigation.focusLabel}</strong></div><div><button type="button" disabled={!navigation.trail.length} onClick={navigation.rewind}>BACK ONE RELATION</button><button type="button" disabled={!navigation.trail.length} onClick={navigation.clearTrail}>RESET TO FOCUS</button></div></header><div className="bf-machine-trail__steps"><span className="bf-machine-trail__origin">{focus?.label ?? navigation.focusLabel}</span>{navigation.trail.map((step, index) => <span className="bf-machine-trail__step" key={`${step.edgeKey}-${index}`}><b>{step.direction === "forward" ? "→" : "←"}</b><small>{step.relation}</small><strong>{getLabMachineNode(step.to)?.label ?? step.to}</strong></span>)}</div></section>;
}
