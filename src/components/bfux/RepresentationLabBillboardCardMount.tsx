...
const researchSelector = '.bf-machine-node[data-node-id="research"]';
...
export function RepresentationLabBillboardCardMount() {
  ...
  useEffect(() => {
    ...
    const align = () => {
      ...
      const research = currentHost.querySelector<HTMLElement>(researchSelector);
      ...
      const researchShellRect = research?.querySelector<HTMLElement>(".bf-machine-node__shell")?.getBoundingClientRect();
      ...
      const authoredGapY = Number.parseFloat(getComputedStyle(billboard).getPropertyValue("--billboard-gap-y")) || 0;
      const isFullProjection = machine?.dataset.resolution === "mid" || machine?.dataset.resolution === "full";
      const productsBottom = productsShellRect?.bottom ?? productsRect.bottom;
      const researchTop = researchShellRect?.top ?? researchRect?.top;
      const mirroredStackGap = isFullProjection && researchTop !== undefined
        ? Math.max(0, (researchTop - productsBottom) / scaleY)
        : authoredGapY;
      const left = productCenterX - billboard.offsetWidth / 2;
      const top = productTop - billboardBottomOffset - mirroredStackGap;

      billboard.style.setProperty("left", `${left}px`, "important");
      billboard.style.setProperty("top", `${top}px`, "important");
      billboard.dataset.billboardAnchor = isFullProjection ? "products-above-mirrored-gap" : "products-above";
    ...
    const observeCurrentGeometry = () => {
      ...
      if (research) resizeObserver.observe(research);
    };
