...
export type HopfFiberCanvasProps = {
  animate?: boolean;
  phase?: number;
  fiberPhase?: number;
...
export const HOPF_FIBER_SAMPLES = 76;
export const HOPF_MAX_FIBERS = 19;
...
function sampleFiber(seed: HopfSeed, phase: number, reduced: boolean): Vec3[] {
  ...
  for (let sample = 0; sample <= HOPF_FIBER_SAMPLES; sample += 1) {
    const fiberPhase = (sample / HOPF_FIBER_SAMPLES) * TAU;
...
export function HopfFiberCanvas({
  animate = true,
  phase = 0,
  fiberPhase = 0,
  cameraOffsetDeg = 0,
  fiberLimit = HOPF_MAX_FIBERS,
  className,
  ariaLabel = "Hopf fibration stereographic projection",
}: HopfFiberCanvasProps) {
  ...
  useEffect(() => {
    ...
    const draw = (time: number) => {
      ...
      const basePhase = animate
        ? ((time % loopDurationMs) / loopDurationMs) * TAU
        : Math.max(0, Math.min(1, phase)) * TAU;
      const reduced = reducedMotion.matches;
      const amplitude = reduced ? 0.34 : 1;
      const cameraOffset = (cameraOffsetDeg * Math.PI) / 180;
      const rotation: [number, number, number] = [
        0.42 + cameraOffset + Math.sin(basePhase) * 0.82 * amplitude,
        -0.32 + Math.sin(basePhase * 2) * 0.22 * amplitude,
        Math.cos(basePhase) * 0.2 * amplitude,
      ];

      const glowX = width * 0.5 + Math.sin(basePhase) * width * 0.035 * amplitude;
      const glowY = height * 0.5 + Math.sin(basePhase * 2) * height * 0.025 * amplitude;
      ...
      const limit = Math.max(1, Math.min(HOPF_MAX_FIBERS, Math.round(fiberLimit)));
      const ordered = seeds
        .slice(0, limit)
        .map((seed) => {
          const points = sampleFiber(seed, basePhase, reduced);
      ...
      ordered.forEach(({ seed, points }, fiberIndex) => {
        ...
        context.strokeStyle = color(seed.phase + basePhase, 0.24 + seed.strength * 0.33);
        ...
        context.strokeStyle = color(seed.phase + basePhase, 0.92);
        context.lineWidth = 1.28 * Math.max(1, dpr * 0.66);
        context.shadowBlur = 11;
        context.shadowColor = color(seed.phase + basePhase, 0.76);
        context.stroke();
      });
    ...
    const render = (time: number) => {
      draw(time);
      if (animate) animationFrame = window.requestAnimationFrame(render);
    };
  ...
  }, [animate, cameraOffsetDeg, fiberLimit, fiberPhase, phase]);
