...
# Cross-layer historical lineage integration

Projects, products, services, testbeds, and artifacts may carry lineage relations, but a work entity must never visually appropriate the authority of a historical figure.

Required behavior:

- show the work entity type and maturity before historical references;
- label historical relations as `foundation`, `method`, `precedent`, `critical counterpoint`, `neighboring lineage`, or `comparison`;
- place historical works in the Lineage tab or Halo lineage filter rather than in product endorsement areas;
- prohibit quotations or portraits as decorative authority signals;
- require an explicit non-claim and citation status;
- keep product evidence and validation separate from intellectual lineage.

A product page may say “uses methods from graph theory and operator analysis.” It may not imply that a cited mathematician validates the product.
